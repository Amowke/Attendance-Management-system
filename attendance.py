...
def exportCsv():
  try:
    if len(mydata)<1:
      ...
      return False
    fln = filedialog.asksaveasfilename(initialdir=os.getcwd(),title="Open CSV",filetypes=(("CSV File","*.csv"),("ALL File","*.*")),parent=app)
...
#Attendance Status
attendance_lbl = CTkLabel(left_inside_frame,text="Attendace Status:",font=("Arial", 13,"bold"),text_color="black",fg_color="white")
...
update_btn = CTkButton(left_inside_frame,text="Update",font=("Arial",13,"bold"))
update_btn.grid(row=4,column=2,padx=10,pady=5)

reset_btn = CTkButton(left_inside_frame,text="Reset",command=reset_data,font=("Arial",13,"bold"))
reset_btn.grid(row=4,column=3,padx=10,pady=5)
...
app.AttendanceReportTable.bind("<ButtonRelease>",get_cursor)
