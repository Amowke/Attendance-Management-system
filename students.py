...
def add_data():
  if app.var_dep.get()=="Select Department" or app.var_std_name.get()=="" or app.var_std_id.get()=="":
    ...
    try:        
      ...
      fetch_data()
      conn.close()
      messagebox.showinfo("Sucess","Student details has been added successfully",parent=app)                                                                             
  ...
  if app.var_dep.get()=="Select Department" or app.var_std_name.get()=="" or app.var_std_id.get()=="":
    ...
    try:
      ...
      if Update>0:            
        ...
        if not Update:
          return
      messagebox.showinfo("Sucess","Student details has been updated successfully",parent=app)
      conn.commit()
      fetch_data()
      conn.close()    
    except Exception as es:
...
def delete_data():
  if app.var_std_id.get()=="":
    ...
    try:
      ...
      fetch_data()
      conn.close()
      messagebox.showinfo("Delete","Student details has been deleted sucessfully",parent=app)
    except Exception as es:
      messagebox.showerror("Error",f"Due To :{str(es)}",parent=app)
...
departments = ["Select Department","Computer Science","Mathematics","Acturial Science","Physics","BioChemistry"]
...
radiobtn1 = CTkRadioButton(class_student_frame,text="Take Photo Sample",value="Yes", variable=app.radiovar)
...
update_photo_btn = CTkButton(class_student_frame,text="Update Photo Sample",width=45,font=("Arial",13,"bold"))
...
#=============Search System================
search_lbl = CTkLabel(right_frame,text="Search System",font=("Arial", 15,"bold"),text_color="black")
search_lbl.grid(row=0,column=0,padx=10,pady=5,sticky=W)

search_frame = CTkFrame(right_frame,fg_color="white", border_width=2,border_color="black",width=750, height=100)
search_frame.grid(row=1,column=0,padx=10,sticky=W)

#Search By
search_by_lbl = CTkLabel(search_frame,text="Search By:",font=("Arial", 13,"bold"),text_color="black",fg_color="white")
search_by_lbl.grid(row=0,column=0,padx=10,pady=10,sticky=W)

search_by_combo = CTkComboBox(search_frame,values=["Select","Student ID","Student Name"])
...
search_btn = CTkButton(search_frame,text="Search",width=19,font=("Arial",13,"bold"))
search_btn.grid(row=0,column=3,padx=5,pady=5)

showall_btn = CTkButton(search_frame,text="Show All",width=19,font=("Arial",13,"bold"))
showall_btn.grid(row=0,column=4,padx=5,pady=5)


#===========table================
table_frame=Frame(right_frame,bd=2,bg="white",relief=RIDGE)
table_frame.place(x=10,y=100,width=770,height=410)
